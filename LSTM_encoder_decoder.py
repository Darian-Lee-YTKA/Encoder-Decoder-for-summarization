...
class LSTM_Encoder_decoder(nn.Module):
    def __init__(self, h_dim, embed_dim, n_head, n_layer, vocab_size, max_seq_len):
        ...
        self.decoder_layer = nn.TransformerDecoderLayer(
            batch_first=True, d_model=h_dim * 2, nhead=n_head, dropout=.3,
            dim_feedforward=h_dim * 2, norm_first=True)
        self.encoder = nn.LSTM(input_size=embed_dim, hidden_size=h_dim, num_layers=n_layer,
                               batch_first=True, dropout=.3, bidirectional=True)
        # ^ encoder is now a bilstm
        ...
        self.embeddings = nn.Embedding(num_embeddings=vocab_size, embedding_dim=embed_dim,
                                       padding_idx=0)  # zero is our pad index. Always and forever 🕊️
        self.position_embedding_table = nn.Embedding(max_seq_len, embed_dim)
        self.fc_out = nn.Linear(h_dim*2, vocab_size)
        # description of the model params which we will use to save in our results
        self.description = (
            f"h_dim={h_dim*2}, embed_dim={embed_dim}, n_head={n_head}, "
